...
> 在 **TRAE Work CN** 中运行的多智能体投资分析系统 —— 由 14 个专业智能体角色化协作，完成 A 股全维度调研、辩论、风控与决策，并自动生成 HTML 投资报告。

## 项目简介

本项目是一个运行在 **TRAE Work CN** 中的**多智能体（Multi-Agent）分析框架**。

通过 TRAE Work CN 的技能系统（Skill）和智能体调度能力，14 个角色化的专业智能体分工协作，对 A 股个股进行全维度调研、多空辩论、风控审查，最终由组合经理给出投资建议，并自动生成可视化 HTML 报告。

工作方式：在 TRAE Work CN 中打开本项目 → 用户输入股票代码或分析请求 → TRAE Work CN 自动触发 `stock-analysis` 项目级技能 → 调度 14 个智能体协同完成分析 → 输出 HTML 报告。
...
在 TRAE 的对话窗口中，直接输入以下任一内容即可触发完整的多智能体分析流程：

- **股票代码**：如 `000001`、`600519`、`300750`、`688981`
- **股票名称**：如「分析平安银行股票」「看看贵州茅台股票」
- **自然语言请求**：如「帮我研究一下宁德时代股票，评估买入风险」

TRAE 会自动匹配 `stock-analysis` 项目技能 → 并行调度 7 大分析师 → 多空辩论 → 风控 → 组合经理，最终输出投资决策。

### 4. 观察调度过程

TRAE 会按以下阶段推进，你可以在对话流中实时看到：

1. 7 大分析师并行调研（技术 / 舆情 / 新闻 / 基本面 / 政策 / 资金 / 解禁）
2. 质量门控 + 数据源评估
3. 多头 ⇄ 空头 辩论（1~N 轮）
4. 研究经理裁决 → 交易员出方案
5. 激进 / 保守 / 中立 风控辩论与裁决
...
2. **A 股特殊性**：所有分析完整考虑了 A 股的特殊规则（T+1、涨跌停、政策市、散户占比高等）。
