...
## 附录 B:性能基线(2026-06-27 采集)

> 采集方式:本地 Python 跑真实 9 基金持仓 C-1 工作流,真实计算部分用 `data_tools.portfolio`,subagent 部分 mock 模拟。
...
| HHI | 0.2179(中等分散) |
| 穿透权益 | 73.9% |
| HTML 报告大小 | 4,294 bytes |

### B.2 各步骤耗时

| Step | 内容 | 耗时(ms) | 类型 |
|------|------|---------|------|
| 0 | input_router | 0.03 | 真实 |
| 1 | 7 角色 × 9 基金 = 63 subagent | 3,151.10 | mock(真实 LLM 会更长) |
| 2 | data_quality_auditor | 20.82 | mock |
| 2.5 | portfolio_analyst(HHI + balance) | 0.07 | **真实** |
| 3 | 多空辩论 | 50.66 | mock |
| 4 | 研究经理 | 51.01 | mock |
| 5 | 交易员 | 50.32 | mock |
| 6 | 风险评估(3 路) | 50.24 | mock |
| 7 | 组合经理 | 51.10 | mock |
| 8 | html_renderer | 16.97 | 真实(Jinja2) |
| **合计** | | **3,442.32** | 本地 + mock |

### B.3 与 spec 第 8.4 节估算对照

| 指标 | spec 估算 | 实测(mock) | 偏差 |
|------|----------|-----------|------|
| Step 1 subagent 数 | 63 | 63 | 一致 |
| Step 1 时间 | ~6-8 分钟(分 3 批) | 3.15 秒(mock) | mock 远快于真实 |
| Step 2.5 portfolio | ~100ms(估算) | 0.07ms | 真实计算极快 |
| Step 8 渲染 | ~30 秒 | 16.97ms | 真实 Jinja2 极快 |
| **总时间** | **~25-35 分钟** | **3.44 秒** | mock 占主导 |

### B.4 真实运行预测

真实 LLM 调用时(假设每 subagent 5-15 秒):
- Step 1: 63 × (5-15) / 9 并发 = **35-105 秒**
- Step 3-7: ~30 秒(7 步串行,每步 ~5 秒)
- 其他步骤: < 1 秒
- **真实总时间预估: ~1-3 分钟**(取决于 LLM 响应速度)
- **真实 token 数**: 63 × ~15k = ~945k tokens(与 spec 第 8.4 节一致)

### B.5 P2 Backlog 决策

| 策略 | 是否启用 | 理由 |
|------|---------|------|
| P2-1 上下文裁剪 | ⏸ 暂不启用 | 真实 token < 1M,未触发 800k 阈值 |
| P2-2 角色精简 | ⏸ 暂不启用 | 实测 Step 2.5 仅 0.07ms,真实计算不慢;LLM 部分按需启用 |
| P2-3 全并发 | ⏸ 暂不启用 | TRAE 默认并发度未知,先用分批 |
| P2-4 风险降级 | ⏸ 暂不启用 | Step 6 真实 LLM 也需要 3 路视角 |
| P2-5 共享数据层 | ⏸ 暂不启用 | 等真实 subagent token 数据后再决定 |
| P2-6 代表性筛选 | ⏸ 暂不启用 | 9 基金未达 10 只阈值 |
| P2-7 报告缓存 | ⏸ 暂不启用 | 暂未观察到重复分析场景 |

**结论**:目前 9 基金组合 token / 时间均在合理范围,**P2 优化全部延后**,待真实 LLM 跑通后用真实数据决定。
