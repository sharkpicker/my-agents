...
| 排除代码前缀 `16` | 深交所 LOF |
| 排除代码前缀 `18` | 封闭式基金 |
| 排除代码前缀 `5`（即 `50xxx`, `51xxx`, `56xxx`, `58xxx`） | 上交所 ETF / LOF |
| 排除名称包含「封闭」「场内」 | 双重保险，覆盖名称中的标识 |
| 排除 `type` 字段含「封闭式」「场内 ETF」 | 双重保险 |

**保留类型**：股票型、混合型、债券型、货币型、指数型、QDII、FOF、另类等开放份额（**含定期开放式**，如 6 个月定开债基）。

预计过滤后规模：约 8000~12000 只场外开放式基金。

注：定开基金（定期开放）虽限定申赎窗口期，但本质属于场外开放式公募基金，纳入采集范围。
