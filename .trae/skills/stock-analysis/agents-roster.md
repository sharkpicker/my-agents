...
| 角色 | 描述 | 调度方式 |
|------|------|----------|
| 组合分析师 | 整合所有单标的报告,做组合层面诊断(集中度/行业暴露/相关性/申赎压力/风格漂移) | 在 `workflow-portfolio.md` Step 2 中通过 Task 工具调度,提示词中说明角色 |

**注**: 没有固定的 agent 文件,而是通过 Task 工具的 prompt 明确告知 subagent 它的角色。
