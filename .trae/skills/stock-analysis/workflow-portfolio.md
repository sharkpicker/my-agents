...
- **组合工作流的输入识别和处理流程与单标的不同,必须严格按本文件 Step 1-12 顺次执行,不可跳过任何标有"必做"的步骤**。
...
这个数字决定后续 Step 4 的分批策略。

---

## Step 2: 用户风险偏好采集(组合工作流 C-1 / C-3 必做)

**目标**: 在跑每只标的前,先拿到用户的"风险等级 / 投资期限 / 偏好品类 / 排除品类",
让后续的组合诊断(Step 5)和候选推荐(Step 9)有据可依。
...
### 2.2 第二步: 缺失字段用 AskUserQuestion 反问
...
- C-2 全股票组合(股票不做资产配置 gap 分析)
- 单只基金分析(走 B 单基金工作流,不需要 Step 2)
...
**缓存机制**: 所有 `fund info/performance/manager/holdings/flows/nav/news` 命令内置本地缓存检查，如果本地已有未过期数据会自动跳过网络请求并输出 `[缓存命中]`。各类型数据的缓存有效期：

| 数据类型 | 缓存有效期 | 说明 |
|---------|-----------|------|
| info/holdings/manager/flows | 24 小时 | 日频更新即可 |
| performance | 12 小时 | 盘中可变 |
| nav | 4 小时 | 交易日 15:00 后更新 |
| news | 2 小时 | 时效性最强 |

使用 `--force` 参数可强制绕过缓存（如交易日尾盘需要最新净值时）。
...
对每只基金/股票执行增量拉取，参考 workflow-fund.md / workflow-stock.md 的 Step 2 数据拉取清单。**重复拉取已有数据的命令会被缓存自动跳过，无需手动判断**。

### 3.3 分析师视角

分析师读取 `data/funds/<code>/` 或 `data/stocks/<code>/` 下的已有文件，如发现数据缺失可自行补充拉取（同样受益于缓存机制）。
...
## Step 5: 组合层面分析(主对话收集 subagent 输出后,调度 1 个"组合分析师"subagent)
...
### 5.2 C-1 全基金专项维度(只在 funds.length > 0 且 stocks.length == 0 时输出)
...
### 5.4 C-3 混合组合专项维度(只在 funds.length > 0 且 stocks.length > 0 时输出)⭐ 核心
...
## Step 6: Gap 分析(C-1 / C-3 必做)

**目标**: 把 Step 5 输出的"组合层诊断"和 Step 2 输出的"用户目标配置"对齐,算出每个
资产大类的 gap,并标记需要加仓 / 减仓的品类。**为 Step 9 的"自动补/换基金"提供输入**。

**前提**: 已完成 Step 2,`data/portfolios/<user_id>/prefs.json` 已落盘。
...
# cash_amount: 余额宝/现金管理类资产金额,使百分比基于"总资产 = 基金 + 现金"计算
cash_amount = sum(h["amount"] for h in holdings if h.get("category") == "cash")
current_alloc = compute_current_allocation(positions, cash_amount=cash_amount)
...
```python
current, gaps, underweight, overweight = compute_gap(positions, target_alloc, cash_amount=cash_amount)
...
- 用户明确说"不需要调整 / 只看诊断"
- Step 2 未跑(没有 prefs.json)
- C-2 全股票组合(股票不做资产配置 gap 分析)

---

## Step 7: 多空辩论(可选,组合 > 5 只时强烈建议)
...
## Step 8: 投资经理裁决 + 交易方案（合并版）

调用 `investor-manager` subagent，替代原 Step 8（研究经理）+ Step 9（交易员），合并输出投资计划 + 交易方案。

```
Task({description: "组合投资经理裁决", prompt: "你是 investor-manager(投资经理)。读取 agents/investor-manager.agent.md,基于组合分析报告 + 多空辩论记录 + Gap 分析结果输出:整体评级(战略再平衡/维持现状/大幅调整)、核心逻辑、多空评估、分标的操作建议、具体交易方案(含每只标的的交易方向/价位/仓位/时间窗口)、风险控制措施。保存到 reports/<日期>/portfolio/portfolio_investment_plan.md。", subagent_type: "general_purpose_task"})
```

---

## Step 9: 候选基金深度推荐(C-1 / C-3 必做)

**目标**: 基于 Step 6 Gap 分析的 underweight 品类,从全量基金库中筛选候选基金,
为每只候选跑 7 大基金分析师 + 1 轮类内多空辩论,用 `parse_quality_from_reports()` 规则化生成质量分,
与原 _score_fund 名称分按 3:7 融合,输出"质量分 + 推荐理由"双重信号的 `portfolio_fund_recommendations.md`。
...
**调度成本**:
- 单类 underweight: 7 分析师 × 3 候选 + 2 辩论 = 23 subagent
- 2 类 underweight: 2 × 23 = 46 subagent（约 1-2 分钟）

### 9.1 主对话预生成候选列表（每类 3 只）
...
candidates_by_cat = screen_replacement_funds(
    ...
    prefs=prefs,
    per_category=3,  # 每类筛选 Top-3 候选（原 5 只）
)
candidates_by_cat = {cat: [c.to_dict() for c in cands] for cat, cands in candidates_by_cat.items()}
```

### 9.2 调度 fund-recommender subagent

```
Task({
  description: "候选基金深度推荐",
  prompt: "你是 fund-recommender。读取 agents/fund-recommender.agent.md。
...
- Step 10 (交易员方案): 必须引用推荐汇总,基于推荐的候选基金制定具体的申购/赎回方案
- Step 11 (风控): 必须引用推荐汇总,审查替换标的风险
- Step 11 (组合经理): 必须读推荐汇总,包含"推荐补/换基金的深度评估"章节
- Step 12 (HTML 渲染): 必须渲染"质量分组成表"和"深度报告路径表"

### 9.6 跳过条件 ⚠️

**只有满足以下条件之一时才可以跳过 Step 9**:

1. `underweight` 为空(没有低配品类需要补)
2. C-2 全股票组合(股票不做基金推荐)
3. 用户明确说"不需要推荐基金 / 只看诊断"
4. `data/funds/_meta/fund_list.json` 不存在且无法获取
...
调用 `consolidated-risk` subagent，替代原 Step 11（激进+保守+中立 3 个风控），直接给出风控结论。

```
Task({description: "组合风控审查", prompt: "你是 consolidated-risk(风控分析师)。读取 agents/consolidated-risk.agent.md,基于投资方案 + 组合分析报告 + Gap 分析结果输出:审查结论(通过/有条件通过/不通过)、综合风险评估(6维度)、风险收益比、参数调整建议、具体风控措施、预警信号。保存到 reports/<日期>/portfolio/portfolio_risk.md。", subagent_type: "general_purpose_task"})
```
...
```
Task({description: "组合经理最终报告", prompt: "你是 portfolio-manager(组合经理)。读取 agents/portfolio-manager.agent.md,综合所有材料(组合分析报告 + 多空辩论 + 研究计划 + 基金推荐报告 + 交易方案 + 风控报告)输出最终组合诊断报告:数据源评估、组合整体评级、调整方向、核心观点、多维度分析摘要(根据组合类型 C-1/C-2/C-3 选择对应维度)、投资逻辑、具体操作建议(分标的)、推荐补/换基金清单、关注要点、免责声明。保存到 reports/<日期>/portfolio/portfolio_final.md。", subagent_type: "general_purpose_task"})
```

---

## Step 12: HTML 报告生成与保存(主对话执行)
...
[Step 1] 主对话识别所有标的(代码+名称+金额+占比+收益)
    ...
    ↓
[Step 2] ⭐ C-1/C-3 必做: 主对话直接采集(文本解析 + AskUserQuestion) → prefs.json
    ↓
...
[Step 5] 调度 1 个组合分析师 subagent 做组合层面诊断(自适应 C-1/C-2/C-3 维度)
    ↓
[Step 6] ⭐ C-1/C-3 必做: 主对话内联算 gap(当前 vs 目标) → portfolio_gap.md
    ↓
[Step 7] 多空辩论(bull + bear subagent 并行,保留 1 轮)
    ↓
...
**subagent 数量对比（9只基金 + 2类 underweight）**：
- 优化前：~146 个
- 优化后：~115 个（减少约 21%）
