...
**Spec**: `docs/superpowers/specs/2026-07-01-portfolio-workflow-redesign-design.md`
...
- fund-recommender subagent 保留为数据底座但不再被 portfolio-manager 输出引用
...
# 清除类清单: 调用 classify_exit_reasons(holdings, fund_reports, gap_report, prefs)
```
...
- Step 10 (交易员方案): 可引用补充类清单,基于品类缺口制定具体的申购/赎回方案
- Step 11 (风控): 可引用清除类清单,审查被清出标的风险
- Step 11 (组合经理): 必须读 gap_report + classify_exit_reasons 输出,在第六章给出"补充/清除分类清单"
- Step 12 (HTML 渲染): 必须渲染"操作建议(P0/P1/P2/P3)"和"数据源评估"两个新模块
...
```
Task({description: "组合经理最终报告", prompt: "你是 portfolio-manager(组合经理)。读取 agents/portfolio-manager.agent.md,综合所有材料(组合分析报告 + 多空辩论 + 研究计划 + 交易方案 + 风控报告)输出最终组合诊断报告。
...
保存到 reports/<日期>/portfolio/portfolio_final.md。", subagent_type: "general_purpose_task"})
```
